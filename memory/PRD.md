...
4. **RLM** (submenus **RLM Fase 1** e **RLM Fase 2**) — **Fase 1**: novo processo pode ser **criado a partir de um contrato License In** (prefill de projeto/título/artista). Workflow de 8 etapas com stepper clicável + modo Corrigir. Etapa Escritório: dropdown de bancos, participantes com **dados bancários obrigatórios** (bloqueiam o avanço) e alocação automática a partir do artista principal. Múltiplos **ISRC/GRID atrelados a faixa**. Etapa 5 organizada em seções numeradas (Vendor / Callback / Documento assinado). Envio ao escritório (e-mail/mailto/link + "aguardando há X dias"); formulário público responsivo. Documento de Callback = template de Envio ao Exterior (PDF) + upload do assinado (object storage). Pendências. **Fase 2** (`/rlm`): Cadastro de Royalties/Direitos.

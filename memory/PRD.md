...
4. **RLM** (submenus **RLM Fase 1** e **RLM Fase 2**) — **Fase 1**: workflow de 8 etapas. Envio ao escritório com **Enviar por e-mail** (Resend ou abre o cliente de e-mail via mailto), **Reenviar lembrete** e **Copiar link**; formulário público `/form/escritorio/:token` com royalties + participantes (alocação ≤ total). **Sub-fase C**: documento de Callback preenchido + impressão PDF + upload/link do assinado com carimbo "anexado em DD/MM". Pendências (`/rlm/pendencias`). **Fase 2** (`/rlm`): Cadastro de Royalties/Direitos.

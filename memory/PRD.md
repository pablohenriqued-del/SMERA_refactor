...
4. **RLM** (submenus **RLM Fase 1** e **RLM Fase 2**) — **Fase 1**: workflow de 8 etapas com stepper clicável (etapas anteriores em leitura + modo **Corrigir** com log no histórico). Etapa Escritório: dropdown de bancos do Brasil, participantes com **dados bancários** e alocação a partir do total do **artista principal** (subtração ao vivo), múltiplos **ISRC/GRID**. Envio ao escritório (e-mail/mailto/copiar link + "aguardando retorno há X dias"); formulário público responsivo. Documento de **Callback = template de Envio ao Exterior** (PDF) + **upload do assinado (object storage)** ou link. Pendências. **Fase 2** (`/rlm`): Cadastro de Royalties/Direitos.

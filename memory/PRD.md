...
4. **RLM** (submenus **RLM Fase 1** e **RLM Fase 2**) — **Fase 1**: workflow de 8 etapas com **stepper clicável** (as 8 etapas cabem na tela; clicar em uma etapa mostra seus dados em **somente leitura**, etapa atual permanece editável). Envio ao escritório com **Enviar por e-mail** (Resend ou mailto), **Reenviar lembrete**, **Copiar link** e indicador **"aguardando retorno há X dias"**; formulário público com royalties + participantes (alocação ≤ total). **Sub-fase C**: documento de Callback + PDF + upload (object storage)/link do assinado com carimbo "anexado em DD/MM". Pendências (`/rlm/pendencias`). **Fase 2** (`/rlm`): Cadastro de Royalties/Direitos.

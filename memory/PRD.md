...
4. **RLM** — **Fase 1** (`/rlm/processos`): workflow de 8 etapas (stepper, formulários por etapa, validação CPF/CNPJ, avanço/devolução, histórico). **Envio ao escritório**: A&R envia link único por e-mail (Resend) ou "Copiar link"; **formulário público** sem login em `/form/escritorio/:token` (reenviável). **Alocação de royalties**: soma dos participantes não pode ultrapassar o total da faixa (visual Total/Alocado/Faltam/Excedeu + validação backend). **Pendências** (`/rlm/pendencias`). **Fase 2** (`/rlm`): Cadastro de Royalties/Direitos. Botão "Pronto" cria o registro da Fase 2.
